 ...
 */
function buildTradeSetup(perTimeframe, weightedScore, risk) {
  ...
  if (direction === 'long') {
    stopLoss = nearestSupport
      ? Number((nearestSupport.price - 0.5 * atr15).toFixed(2))
      : Number((entryMid - 1.5 * atr15).toFixed(2));
  } else {
    stopLoss = nearestResistance
      ? Number((nearestResistance.price + 0.5 * atr15).toFixed(2))
      : Number((entryMid + 1.5 * atr15).toFixed(2));
  }

  const riskDistance = Math.abs(entryMid - stopLoss);
  const sign = direction === 'long' ? 1 : -1;
  const takeProfits = [1, 2, 3].map((r) => Number((entryMid + sign * r * riskDistance).toFixed(2)));
  ...
  return {
    ...
    entryZone: {
      ...
      label: `@${entryHigh.toFixed(2)}-${entryLow.toFixed(2)}`,
    },
    stopLoss,
    takeProfits,
    runner: 'open',
...
function buildConfluence(perTimeframe) {
  const { '15m': m15, '1h': h1, '1d': d1 } = perTimeframe;
  ...
  const tradeSetup = buildTradeSetup(perTimeframe, weightedScore, risk);
