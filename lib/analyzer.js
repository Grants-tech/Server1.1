...
function analyzeTimeframe(timeframe, candles, decimals = 3) {
  ...
  const sr = findSupportResistance(swings, price, 0.0015, decimals);
  const patterns = detectPatterns(candles);
  const fib = computeFibLevels(candles, Math.min(candles.length, 80), decimals);
  ...
  const macdDecimals = Math.max(decimals + 1, 4);
  ...
  return {
    ...
    trend: {
      ...
      lastSwingHighs: trendInfo.lastSwingHighs.map((s) => ({ price: Number(s.price.toFixed(decimals)), time: s.time })),
      lastSwingLows: trendInfo.lastSwingLows.map((s) => ({ price: Number(s.price.toFixed(decimals)), time: s.time })),
    ...
    movingAverages: {
      ema20: last(ema20) != null ? Number(last(ema20).toFixed(decimals)) : null,
      ema50: last(ema50) != null ? Number(last(ema50).toFixed(decimals)) : null,
      ema200: last(ema200) != null ? Number(last(ema200).toFixed(decimals)) : null,
    ...
    momentum: {
      ...
      macdHistogram: last(macdRes.histogram) != null ? Number(last(macdRes.histogram).toFixed(macdDecimals)) : null,
    ...
    volatility: {
      atr14: last(atr14) != null ? Number(last(atr14).toFixed(decimals)) : null,
      bollingerUpper: last(bb.upper) != null ? Number(last(bb.upper).toFixed(decimals)) : null,
      bollingerMiddle: last(bb.middle) != null ? Number(last(bb.middle).toFixed(decimals)) : null,
      bollingerLower: last(bb.lower) != null ? Number(last(bb.lower).toFixed(decimals)) : null,
    },
