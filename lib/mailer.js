 ...
 * Used only to verify SMTP credentials/delivery are working — not part
 * of the normal analysis cycle.
 */
async function sendTestEmail() {
  const to = process.env.EMAIL_TO;
  const from = process.env.EMAIL_FROM || process.env.SMTP_USER;
  if (!to) {
    throw new Error('EMAIL_TO is not set — nowhere to send the test email.');
  }
