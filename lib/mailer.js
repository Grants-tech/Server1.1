...
function formatSignalEmail(result) {
  ...
  const symbolLabel = result.symbol || 'XAU/USD';
  const symbolCompact = symbolLabel.replace('/', '');
  const dirLabel = ts.direction === 'long' ? 'LONG' : 'SHORT';
  const emoji = ts.direction === 'long' ? '🟢' : '🔴';
  const subject = `${emoji} High-Confidence ${symbolCompact} ${dirLabel} Signal — score ${result.weightedScore}`;

  const tpLines = ts.takeProfits.map((tp, i) => `TP${i + 1}: ${tp}`).join('\n');

  const text = [
    `${symbolLabel} ${dirLabel} — high-confidence intraday signal`,
  ...
  const html = `
    <h2>${emoji} ${symbolLabel} ${dirLabel} — high-confidence intraday signal</h2>
...
function formatBounceEmail(bounce, price, symbolLabel = 'XAU/USD') {
  const symbolCompact = symbolLabel.replace('/', '');
  const dirLabel = bounce.direction === 'long' ? 'LONG' : 'SHORT';
  const subject = `⚠️ Counter-Trend Bounce ${symbolCompact} ${dirLabel} — off ${bounce.keyLevel.side} ${bounce.keyLevel.price}`;

  const tpLines = bounce.takeProfits.map((tp, i) => `TP${i + 1}: ${tp}`).join('\n');

  const text = [
    `${symbolLabel} ${dirLabel} — counter-trend bounce/reversal signal (higher risk)`,
    `Price: ${price}`,
    `Key level: ${bounce.keyLevel.side} at ${bounce.keyLevel.price} (${bounce.keyLevel.touches} touches)`,
    '',
    `Entry: ${bounce.entryZone.label}`,
    `SL: ${bounce.stopLoss}`,
    tpLines,
    `TP: Open (trail remainder)`,
    '',
    'Why:',
    ...bounce.reasoning.map((n) => `- ${n}`),
    '',
    bounce.riskWarning,
    '',
    'Automated technical-analysis output for informational purposes only. Not financial advice.',
  ].join('\n');

  const html = `
    <h2>⚠️ ${symbolLabel} ${dirLabel} — counter-trend bounce/reversal (higher risk)</h2>
    <p><b>Price:</b> ${price}<br/>
       <b>Key level:</b> ${bounce.keyLevel.side} at ${bounce.keyLevel.price} (${bounce.keyLevel.touches} touches)</p>
    <table cellpadding="4" style="border-collapse:collapse">
      <tr><td><b>Entry</b></td><td>${bounce.entryZone.label}</td></tr>
      <tr><td><b>SL</b></td><td>${bounce.stopLoss}</td></tr>
      ${bounce.takeProfits.map((tp, i) => `<tr><td><b>TP${i + 1}</b></td><td>${tp}</td></tr>`).join('')}
      <tr><td><b>TP: Open</b></td><td>trail remainder</td></tr>
    </table>
    <p><b>Why:</b></p>
    <ul>${bounce.reasoning.map((n) => `<li>${n}</li>`).join('')}</ul>
    <p style="color:#b45309"><b>${bounce.riskWarning}</b></p>
    <p style="color:#888;font-size:12px">Automated technical-analysis output for informational purposes only. Not financial advice.</p>
  `;

  return { subject, text, html };
}

async function sendBounceEmail(bounce, price, symbolLabel) {
  const { subject, text, html } = formatBounceEmail(bounce, price, symbolLabel);
  await sendEmail({ subject, text, html });
}
...
module.exports = { sendSignalEmail, sendTestEmail, sendBounceEmail, formatSignalEmail, formatBounceEmail };