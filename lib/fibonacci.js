...
function computeFibLevels(candles, lookback = 60, decimals = 3) {
  ...
  return {
    ...
    retracements: retracements.map((r) => ({ ...r, price: Number(r.price.toFixed(decimals)) })),
    extensions: extensions.map((r) => ({ ...r, price: Number(r.price.toFixed(decimals)) })),
  };
}
